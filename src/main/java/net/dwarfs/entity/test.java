...
// public class DwarfEntity extends PassiveEntity implements InventoryOwner, Npc, InteractionObserver, VillagerDataContainer {
...
// // SensorType.NEAREST_ITEMS, SensorType.NEAREST_BED, SensorType.HURT_BY, SensorType.VILLAGER_HOSTILES, SensorType.VILLAGER_BABIES, SensorType.SECONDARY_POIS, SensorType.GOLEM_DETECTED);
...
// brain.setTaskList(Activity.MEET, VillagerTaskListProvider.createMeetTasks(villagerProfession, 0.5f),
...
// // ((DataResult) dataResult).resultOrPartial(LOGGER::error).ifPresent(this::setVillagerData);
// }
...
// VillagerType villagerType = d < 0.5 ? VillagerType.forBiome(serverWorld.getBiomeKey(this.getBlockPos()))
// : (d < 0.75 ? this.getVillagerData().getType() : ((DwarfEntity) passiveEntity).getVillagerData().getType());
// DwarfEntity villagerEntity = new DwarfEntity(EntityInit.DWARF, serverWorld, villagerType);
