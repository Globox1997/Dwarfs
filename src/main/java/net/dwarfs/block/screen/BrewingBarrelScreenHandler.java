package net.dwarfs.block.screen;
...
public class BrewingBarrelScreenHandler extends ScreenHandler {
    ...
    public BrewingBarrelScreenHandler(int syncId, PlayerInventory playerInventory, Inventory inventory, PropertyDelegate propertyDelegate) {
        super(BlockInit.BREWING_BARREL, syncId);
        int i;
        BrewingBarrelScreenHandler.checkSize(inventory, 5);
        BrewingBarrelScreenHandler.checkDataCount(propertyDelegate, 2);
    ...
    public ItemStack transferSlot(PlayerEntity player, int index) {
        ...
        if (slot != null && slot.hasStack()) {
            ...
            } else if (FuelSlot.matches(itemStack) ? this.insertItem(itemStack2, 4, 5, false) || this.ingredientSlot.canInsert(itemStack2) && !this.insertItem(itemStack2, 3, 4, false)
                    : (this.ingredientSlot.canInsert(itemStack2) ? !this.insertItem(itemStack2, 3, 4, false)
                            : (PotionSlot.matches(itemStack) && itemStack.getCount() == 1 ? !this.insertItem(itemStack2, 0, 3, false)
                                    : (index >= 5 && index < 32 ? !this.insertItem(itemStack2, 32, 41, false)
                                            : (index >= 32 && index < 41 ? !this.insertItem(itemStack2, 5, 32, false) : !this.insertItem(itemStack2, 5, 41, false)))))) {
                return ItemStack.EMPTY;
            }
