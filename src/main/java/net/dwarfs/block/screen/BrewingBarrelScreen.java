...
public class BrewingBarrelScreen extends HandledScreen<BrewingBarrelScreenHandler> {
    private static final Identifier TEXTURE = new Identifier("dwarfs:textures/gui/brewing_barrel.png");
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        int k = ((BrewingBarrelScreenHandler) this.handler).getFuel();
        int l = MathHelper.clamp((18 * k + 20 - 1) / 20, 0, 18);
        if (l > 0) {
            this.drawTexture(matrices, i + 60, j + 44, 176, 29, l, 4);
        }
        if ((m = ((BrewingBarrelScreenHandler) this.handler).getBrewTime()) > 0) {
