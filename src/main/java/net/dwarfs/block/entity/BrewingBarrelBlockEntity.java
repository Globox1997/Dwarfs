package net.dwarfs.block.entity;
...
public class BrewingBarrelBlockEntity extends LockableContainerBlockEntity implements SidedInventory {
    ...
    protected final PropertyDelegate propertyDelegate = new PropertyDelegate() {
        ...
        public int get(int index) {
            ...
            case 1: {
                return BrewingBarrelBlockEntity.this.fuel;
            }
            }
            return 0;
        }
        ...
        public void set(int index, int value) {
            switch (index) {
            case 0: {
                BrewingBarrelBlockEntity.this.brewTime = value;
                break;
            }
            case 1: {
                BrewingBarrelBlockEntity.this.fuel = value;
            }
            }
        }
    ...
    public BrewingBarrelBlockEntity(BlockPos pos, BlockState state) {
        super(BlockInit.BREWING_BARREL_ENTITY, pos, state);
    }
    ...
    public static void tick(World world, BlockPos pos, BlockState state, BrewingBarrelBlockEntity blockEntity) {
        ItemStack itemStack = blockEntity.inventory.get(4);
        if (blockEntity.fuel <= 0 && itemStack.isOf(Items.BLAZE_POWDER)) {
            blockEntity.fuel = 20;
            itemStack.decrement(1);
            BrewingBarrelBlockEntity.markDirty(world, pos, state);
        }
        boolean bl = BrewingBarrelBlockEntity.canCraft(blockEntity.inventory);
        boolean bl2 = blockEntity.brewTime > 0;
        ItemStack itemStack2 = blockEntity.inventory.get(3);
        if (bl2) {
            boolean bl3;
            --blockEntity.brewTime;
            bl3 = blockEntity.brewTime == 0;
            if (bl3 && bl) {
                BrewingBarrelBlockEntity.craft(world, pos, blockEntity.inventory);
                BrewingBarrelBlockEntity.markDirty(world, pos, state);
        ...
        if (!Arrays.equals(bl3, blockEntity.slotsEmptyLastTick)) {
            ...
            if (!(blockState.getBlock() instanceof BrewingBarrelBlock)) {
                return;
            }
            for (int i = 0; i < BrewingBarrelBlock.BOTTLE_PROPERTIES.length; ++i) {
                blockState = (BlockState) blockState.with(BrewingBarrelBlock.BOTTLE_PROPERTIES[i], bl3[i]);
            }
