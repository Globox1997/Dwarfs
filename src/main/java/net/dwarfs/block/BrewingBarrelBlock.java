...
public class BrewingBarrelBlock extends BlockWithEntity {
    ...
    public BrewingBarrelBlock(AbstractBlock.Settings settings) {
        super(settings);
        this.setDefaultState((BlockState) ((BlockState) ((BlockState) ((BlockState) this.stateManager.getDefaultState()).with(BOTTLE_PROPERTIES[0], false)).with(BOTTLE_PROPERTIES[1], false))
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return world.isClient ? null : BrewingBarrelBlock.checkType(type, BlockInit.BREWING_BARREL_ENTITY, BrewingBarrelBlockEntity::tick);
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (blockEntity instanceof BrewingBarrelBlockEntity) {
            player.openHandledScreen((BrewingBarrelBlockEntity) blockEntity);
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
        ...
        if (itemStack.hasCustomName() && (blockEntity = world.getBlockEntity(pos)) instanceof BrewingBarrelBlockEntity) {
            ((BrewingBarrelBlockEntity) blockEntity).setCustomName(itemStack.getName());
        }
    }
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        ...
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (blockEntity instanceof BrewingBarrelBlockEntity) {
            ItemScatterer.spawn(world, pos, (Inventory) ((BrewingBarrelBlockEntity) blockEntity));
        }
